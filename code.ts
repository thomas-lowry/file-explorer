...
// vars for metrics
const localPaints:PaintStyle[] = figma.getLocalPaintStyles();
const localEffects:EffectStyle[] = figma.getLocalEffectStyles();
const localTexts:TextStyle[] = figma.getLocalTextStyles();
...
// component vars
const masterComponents:ComponentNode[] = findComponents();
...
function findInstances() {
	...
	let instanceList = [];
	let allInstances = Array.from(figma.root.findAll(i => i.type === 'INSTANCE') as InstanceNode[]);
	let uniqueInstances = removeDuplicatesBy(i => i.masterComponent.id, allInstances) as InstanceNode[];

	uniqueInstances.forEach(instance => {
		let mcId = instance.masterComponent.id;
		let instances = Array.from(figma.root.findAll(i => i.type === 'INSTANCE' && i.masterComponent.id === mcId) as InstanceNode[]);
		let instanceEntry = {
			'name': instance.name,
...
function findComponents() {
	let components = figma.root.findAll(mc => mc.type === 'COMPONENT') as ComponentNode[];
	return components;
}
